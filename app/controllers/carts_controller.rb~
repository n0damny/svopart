class CartsController < ApplicationController
  ...
  def show
    begin
      ...
      #"Attempt to access invalid cart #{params[:id]}"
      logger.error "Попытка доступа к несуществующей корзине #{params[:id]}"
      redirect_to root_path, :notice => 'Хреновая корзина'
    else
      respond_to do |format|
...
#        'Cart was successfully created.'
        format.html { redirect_to(@cart, :notice => 'Корзина была успешно создана.') }
  ...
  def update
    ...
    respond_to do |format|
      if @cart.update_attributes(params[:cart])
        format.html { redirect_to(@cart, :notice => 'Корзина была успешно изменена.') }
        format.xml  { head :ok }
      else
